...
# CUSTOM CSS
# ─────────────────────────────────────────────
_STYLESHEET = Path(__file__).with_name("static") / "styles.css"


@st.cache_data(show_spinner=False)
def _inline_stylesheet(_stamp: float) -> str:
    """Fallback: the whole stylesheet in a <style> tag, memoised on file mtime."""
    return f"<style>{_STYLESHEET.read_text(encoding='utf-8')}</style>"


def stylesheet_tag() -> str:
    """Link the stylesheet as a real static file rather than inlining it.

    It used to be a 1,300-line string inside this script, which meant ~66KB of CSS
    was serialised down the websocket on *every* rerun — and Streamlit reruns the
    whole script each time a recruiter nudges the score filter. Served from
    static/ instead, the browser caches it once and each rerun carries a ~60-byte
    link tag.

    The mtime query string is a cache-buster: without it an edit to the CSS would
    sit invisible behind the browser cache until a hard refresh.

    Static serving is switched on in .streamlit/config.toml, which Streamlit reads
    relative to the working directory rather than to this file — so launching the
    app from another folder would silently turn it off and serve a completely
    unstyled page. In that case fall back to inlining: slower per rerun, but the
    product still looks like the product.
    """
    try:
        stamp = _STYLESHEET.stat().st_mtime
    except OSError:
        return ""  # stylesheet missing entirely; let the app render bare
    ...
    if static_on:
        return f'<link rel="stylesheet" href="app/static/styles.css?v={int(stamp)}">'
    return _inline_stylesheet(stamp)


st.markdown(stylesheet_tag(), unsafe_allow_html=True)
...
@st.cache_data(show_spinner=False)
def cached_rank(_parsed: list, _jd: dict, cache_key: str) -> list:
    """Score the batch once per (batch, job description) pair.

    Ranking is deterministic in its two inputs, but Streamlit reruns the whole
    script on every widget interaction — so without this, dragging the score
    slider or ticking a checkbox re-scored all 100 CVs for an identical result.

    `_parsed` and `_jd` are underscore-prefixed so Streamlit skips hashing them:
    hashing 100 full resume texts on every rerun would cost more than the
    re-score it saves. `cache_key` carries the real identity instead — a
    signature of the uploaded batch plus a digest of the JD text.
    """
    return rank_candidates(_parsed, _jd)


def batch_signature(files) -> str:
    """Identify a batch of uploads by name and size.

    Cheap and stable: Streamlit hands back the same UploadedFile objects across
    reruns, and a recruiter swapping in a different CV changes either the name
    or the byte count.
    """
...
if PAGE == CTA_TAB and jd_text.strip() and (uploaded_files or st.session_state.get("parsed")):
    ...
    if valid_files:
        ...
        if st.button("🚀 Analyze & Rank Candidates", use_container_width=True, type="primary"):
            ...
            # Stamp the batch so the ranking cache can tell one upload from the next
            # without re-hashing every resume body on every rerun.
            st.session_state["batch_sig"] = batch_signature(valid_files)
    ...
    # Reading the files is the slow part, so that is cached. Scoring is cached too,
    # keyed on the batch and the job description — so editing the JD still re-scores
    # immediately, but moving a filter does not, and neither costs a re-upload.
    ...
    if parsed:
        ...
        jd_digest = hashlib.md5(jd_text.encode("utf-8")).hexdigest()
        rank_key  = f"{st.session_state.get('batch_sig', '')}::{jd_digest}"
        ranked    = cached_rank(parsed, jd_parsed, rank_key)
        ...
        # Each card is ~15 Streamlit elements. Rendering all 100 means ~1,500 live
        # elements rebuilt on every widget interaction, to show a recruiter a stack
        # of cards nobody scrolls to the bottom of. Show the shortlist that actually
        # gets read, and keep the rest one click away.
        CARD_PAGE = 20
        if filtered:
            st.markdown('<div class="section-title">👤 Candidate Details</div>', unsafe_allow_html=True)

        show_all_cards = st.session_state.get("show_all_cards", False)
        visible = filtered if show_all_cards else filtered[:CARD_PAGE]
        ...
        for c in visible:
            ...
            with st.expander(
                ...
                with col_right:
                    ...
                    if show_breakdown:
                        bd = c.get("score_breakdown", {})
                        # One markdown string instead of four st.progress widgets.
                        # Four widgets per card is 400 live elements on a full batch,
                        # every one of them rebuilt and re-sent whenever any filter
                        # moves. This also shows the weights, which the bars did not.
                        st.markdown(
                            bar_row_html("Skills match",        bd.get("skill_score", 0), "50%")
                            + bar_row_html("Relevant experience", bd.get("exp_score", 0), "25%")
                            + bar_row_html("Location fit",        bd.get("loc_score", 0), "15%")
        ...
        # The CSV export below always covers the full filtered list, not just the
        # cards on screen — so collapsing the tail never hides anyone from the sheet.
        if len(filtered) > CARD_PAGE:
            if show_all_cards:
                st.caption(f"Showing all {len(filtered)} candidates.")
                if st.button(f"↑ Collapse to top {CARD_PAGE}", use_container_width=True):
                    st.session_state["show_all_cards"] = False
                    st.rerun()
            else:
                st.caption(
                    f"Showing the top {CARD_PAGE} of {len(filtered)} candidates who passed "
                    f"your filter. The CSV export below includes all {len(filtered)}."
                )
                if st.button(
                    f"↓ Show the remaining {len(filtered) - CARD_PAGE}",
                    use_container_width=True,
                ):
